...
일반 버튼은 NO(Normally Open) 타입으로, 평소 개방 상태이다가 눌리면 closed 상태가 된다.

개방 상태에서 폐쇄 상태로 전환하는 순간 물리적 바운싱(contact bouncing) 현상으로 인해 폐쇄 상태에서 순간적으로 개방 상태로 다시 인식되는 경우가 있다.

디바운싱을 두어 일정시간 이상 버튼을 누르면 단락 상태로 간주하도록 메커니즘을 변경.

**`구현`**

임의의 상수를 두어 해당 시간 이상 폐쇄 유지시 버튼을 누른 것으로 간주한다.

루프에서 스위치가 눌리면 초기에 타임스탬프를 기록하고 이번 루프는 건너뜀. 이후 일정 시간 이상 유지 시 스위치 폐쇄 flag 변경.

```c
#define DEBOUNCE 50 // 디바운스 상수, 50ms 이상 유지시 폐쇄

if(pushedSw == 0 && swPushed(sw1)) {
    if(swMillis == 0){ // 초기에는 timestamp 할당 후 반환
      swMillis = millis();
    } else if(millis() - swMillis > DEBOUNCE){ // 일정 시간 이상 유지시 플래그 변경
      pushedSw = 1;
    }
  }
  ...
