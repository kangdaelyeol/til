...
- [user_app_callbacks (#if !defined(**DA14531_01**) && !defined(**DA14535**))](#user_app_callbacks-if-defined__da14531_01__--defined__da14535__)
...
**[NVDS(Non Volatile Data Storage)](#nvdsnon-volatile-data-storage)**
...
- [connection 핸들러 정의](#connection-핸들러-정의)

- [connection 핸들러 구현](#connection-핸들러-구현)
...
| 특징                          | 설명                                                                                                                                                                          |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 동기식 직렬 통신(Synchronous) | Master로부터 공급되는 SCLK에 맞춰 데이터를 송수신한다                                                                                                                         |
| 전이중(Full-Duplex) 구조      | MOSI / MISO 핀이 동시에 존재하여 구조적으로는 전이중 통신이 가능하지만, Flash 통신은 명령을 전송하고 응답을 받는 구조이므로, 실질적으로 반이중(Half-Duplex) 형식으로 동작한다 |
| 클럭 제한                     | P25Q11U Flash 자체는 최대 50MHz 클럭을 지원하지만, SPI 컨트롤러 설정에 따라 실제 클럭은 제한된다.                                                                             |
...
| 매크로              | 설명                                                                                                                         |
| ------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| NVDS_8BIT_TAGLENGTH | 정의된 경우, 각 Tag의 최대 길이는 256바이트(8비트 길이 필드). 정의되지 않은 경우 최대 65536바이트(16비트 길이 필드)          |
| NVDS_PACKED         | 정의된 경우, 모든 NVDS 데이터는 메모리 상에 경계 없이 연속적으로 저장. 정의되지 않으면 각 Tag 사이에 32비트 경계를 두어 저장 |
| NVDS_READ_WRITE     | 정의되지 않은 경우 GET(read)만 가능. 정의된 경우 GET 외에 PUT(write) / DEL(delete) / LOCK 동작도 제공                        |

**`NVDS 기본 설정 값`**

기본적으로 모든 NVDS 설정값은 true(1)로 설정되어 있다.
...
NVDS API는 헤더파일에 선언부분만 명시되어있다

API 구현 부분은 SDK ROM에 입력되어있어 코드상으로 확인이 불가능하다.

각 메서드의 반환값은 status 를 의미하며, 열거형으로 정의되어있다.
...
디버깅시 반환하는 status 상수값을 통해 상태를 유추할 수 있다.

**`init - 초기화`**

nvds_init은 내부적으로(ble_init 호출시) 같이 실행된다.

@params에서 Not used 부분을 통해 구현 코드 내부적으로 사용되지 않음을 추측할 수 있다.
...
NVDS 메모리를 읽어오는 API는 다음과 같이 두 함수로 정의되어 있다.

```c
// nvds_get nvds_get_func
/**
 ****************************************************************************************
...
\_func 접미사는 관례적으로 wrapper 함수로서 내부적으로 호출되도록 설계된 함수에 사용된다. 따라서 nvds_get 함수를 사용한다.

**`nvds_put - 메모리 입력`**

NVDS write동작은 nvds_put API를 사용하여 입력할 수 있다.
...
임의의 tag와 tag length를 결정하고 데이터를 입력한다.

주석에 나온 설명과 같이 EEPROM에 사용할 수 있는지, locked 상태인지 검사하는 과정을 거친다.

### NVDS 설정

**`__EXCLUDE_ROM_NVDS__ 플래그`**

nvds.c 구현에 의하면 매크로 플래그 정의에 따라 nvds 사용 범위가 결정되어 있다.
...
DA14531이 아니거나 `__EXCLUDE_ROM_NVDS__` 매크로가 정의된 경우,
`NVDS_READ_WRITE == 1` 조건 하에 `nvds_put_func` 와 `nvds_del_func` 가 `NVDS_FAIL` 을 반환하는 대체 구현이 정의되어 있다. (`nvds_init_func` 는 `NVDS_OK` 반환)

**`ROM 전용 설정`**

jump_table.c 에서 보면 nvds 설정은 rom전용으로 되어있는 것을 확인할 수 있다.

```c
/*
 * TYPE DEFINITIONS - nvds.h
 */
...
따라서 DA14531 기본 설정에서는 ROM에 이미 구현된 nvds 함수를 그대로 사용하며, 사용자가 직접 nvds 내부 구현을 변경할 수 없다.

---

## BLE Connection

BLE connection을 위해선 다양한 콜백 함수 정의 및 설정이 필요하다.

### connection 핸들러 정의

BLE connection 발생 시 기본 동작 외에 나머지 동작을 사용자가 설정하는데, 이에 대해 콜백 함수를 선언해야 한다.

SDK 내부는 `app_process_catch_rest_cb` 라는 이름으로 핸들러를 참조하므로, 이 이름을 사용자가 구현할 함수명으로 연결하는 **함수명 매핑 매크로**를 정의해야 한다.

`user_callback_config.h` 파일에서 정의한다.

- 변수 이름 정의부에서 user\_ 접두사가 붙은 변수명은 사용자가 임의로 정의한 변수임을 의미한다.
...
SDK 내부에서 `app_process_catch_rest_cb`를 호출하면 실제로는 사용자가 구현한 `user_catch_rest_hndl` 함수가 실행된다.

**`app_process_catch_rest_cb 구조`**

`app_process_catch_rest_cb` 인터페이스는 내부적으로 다음과 같이 정의되어 있다.
...
`app_process_catch_rest_cb` 콜백을 입력받는 구조체 `app_entry_catch_rest` 타입은 다음과 같이 정의되어 있다.
...
| 파라미터 | 타입          | 설명                                                              |
| -------- | ------------- | ----------------------------------------------------------------- |
| msgid    | ke_msg_id_t   | 수신된 메시지의 ID. 어떤 이벤트가 발생했는지 식별하는 데 사용     |
| param    | void const \* | 메시지 데이터 포인터. 이벤트 종류에 맞는 구조체로 캐스팅하여 사용 |
| dest_id  | ke_task_id_t  | 메시지를 수신하는 태스크 ID                                       |
| src_id   | ke_task_id_t  | 메시지를 송신한 태스크 ID                                         |

- `param` 포인터는 `void *` 타입이므로, `msgid` 에 따라 해당 이벤트의 구조체 타입으로 캐스팅(casting)되어야 실제 데이터에 접근할 수 있다.

- RN(React Native) 앱이 BLE Write로 전송한 데이터도 `param` 을 통해 접근한다.

### connection 핸들러 구현

**`msgid 분기 처리`**

catch_rest 핸들러는 SDK가 처리하지 않은 모든 나머지 메시지를 받는 핸들러다. 따라서 `msgid` 로 이벤트를 식별하여 분기 처리하는 것이 기본 패턴이다.

이벤트 타입이 write인 경우, msgid는 **CUSTS1_VAL_WRITE_IND** 로 입력되어 해당 상수로 조건을 걸어 분기 처리한다.
                           ...
                           ke_task_id_t const src_id)
{
    switch (msgid)
    {
        ...
        {
            // param을 이벤트에 맞는 구조체로 캐스팅하여 데이터 접근
            struct custs1_val_write_ind const *msg_param =
                (struct custs1_val_write_ind const *)(param);
            // msg_param->value 로 RN 앱에서 전송한 데이터 접근
...
**`type casting - custs1_val_write_ind`**

`param` 포인터를 **custs1_val_write_ind** 형으로 캐스팅하여 RN(React Native) 앱이 BLE Write로 전송한 데이터에 접근한다. custs1_val_write_ind 구조는 다음과 같다.
...
| 필드   | 설명                                                                                                        |
| ------ | ----------------------------------------------------------------------------------------------------------- |
| conidx | BLE 연결 인덱스. 다중 연결 환경에서 어떤 연결로부터 온 요청인지 식별                                        |
| handle | 어떤 Characteristic이 Write됐는지를 나타내는 attribute handle                                               |
| length | `value` 배열의 실제 데이터 길이(바이트)                                                                     |
| value  | RN 앱이 BLE Write로 전송한 실제 데이터. `__ARRAY_EMPTY` (유연 배열 멤버)로 선언되어 가변 길이 데이터를 수신 |

- `__ARRAY_EMPTY` 는 크기가 정해지지 않은 **유연 배열 멤버(Flexible Array Member)** 를 선언하는 매크로다. 구조체 자체에 데이터를 포함하지 않고, 메시지 버퍼 뒤에 이어지는 가변 길이 데이터를 포인터처럼 참조한다.

**`Service Discovery / msg_param->handle`**

```text
RN앱 BLE 스택                    DA14531 BLE 스택
    │                                │
    │── Read By Group Type Req ────>│  "Service들 알려줘"
    │<── 응답: Service UUID + handle ─│
    │                                │
    │── Read By Type Request ──────> │  "Characteristic들 알려줘"
    │<── 응답: Char UUID + handle ───│
    │                               │
    │ (이제 handle 매핑 완료)         │
```

RN 앱은 BLE 연결 후 최초 Write를 시도하기 전에 `Service Discovery` 를 수행하여 DA14531의 Service / Characteristic handle 목록을 파악한다.

이 과정에서 DA14531은 GATT DB에 등록된 서비스, 특성, 권한 등의 정보와 attribute handle을 RN 앱에 전달하고, RN 앱은 이 handle을 저장하여 이후 Write 시 사용한다.

RN 앱이 Write 요청을 보낼 때 사용한 attribute handle 값이 `msg_param->handle` 에 담겨 핸들러로 전달된다.

GATT DB는 `user_custs_def.c` 에 구현되어 있으며, `user_custs_def.h` 에 열거형 상수로 정의되어 있다.
...
const struct attm_desc_128 custs1_att_db[CUSTS1_IDX_NB] =
{
    ...
    [SVC1_IDX_SERIAL_VAL]       = {SVC1_SERIAL_UUID_128,                                            // UUID: 어떤 Characteristic인지 식별
                                    ATT_UUID_128_LEN,                                                // UUID 길이
                                    PERM(RD, ENABLE) | PERM(WR, ENABLE) | PERM(WRITE_REQ, ENABLE),  // 권한 정보
                                    DEF_SVC1_SERIAL_CHAR_LEN,                                        // 데이터 길이
                                    0, NULL},
...
**`핸들러 호출 - user_svc1_serial_wr_ind_handler`**
...
`app_easy_timer` 모듈을 사용하여 타이머 콜백을 실행할 수 있다.
...
```c
// app_easy_timer.c
timer_hnd app_easy_timer(const uint32_t delay, timer_callback fn) // typedef void (* timer_callback)(void);
{
    // Sanity checks
...
`app_easy_timer` 인수에서 delay는 10ms 단위로 받으며, timer id를 반환한다.

app_easy_timer에서 호출되는 **`create_timer`** 메서드는 BLE 장치가 sleep 상태인지 검사한다. BLE가 활성 상태이면 즉시 `KE_TIMER_SET`으로 타이머를 등록하고, sleep 상태이면 `arch_ble_force_wakeup()`으로 강제 wakeup 후 `APP_CREATE_TIMER` 메시지를 전송하여 BLE가 완전히 깨어난 뒤 타이머가 생성되도록 한다.

### 타이머 할당

타이머를 예약하면 내부적으로 `set_callback` 을 호출하여 타이머를 할당한다.
...
기본적으로 `APP_TIMER_MAX_NUM` 매크로는 다음과 같이 정의되어 있다.
...
```text
APP_TIMER_MAX_NUM = APP_TIMER_API_LAST_MES - APP_TIMER_API_MES0 + 1
= MES9 - MES0 + 1
= 9 - 0 + 1
= 10
```

따라서 최대 10개의 타이머를 동시에 할당할 수 있다.

### 타이머 취소

`app_easy_timer_cancel` 메서드로 타이머 콜백이 호출되기 전에 취소할 수 있다.

타이머를 취소하기 위해서는 타이머 생성 시 반환된 핸들러 값이 필요하다.
...
타이머를 취소하면 콜백은 호출되지 않으며, 해당 타이머 슬롯(`timer_callbacks[i]`)은 `NULL`로 초기화되어 재사용 가능한 상태가 된다.

`accel_check_timer`와 같이 타이머 핸들러를 변수로 보관하는 경우, 취소 후에는 `EASY_TIMER_INVALID_TIMER`로 초기화하여 유효하지 않은 핸들러로 재취소하는 것을 방지하는 것이 좋다.

```c
if (accel_check_timer != EASY_TIMER_INVALID_TIMER)
{
    app_easy_timer_cancel(accel_check_timer);
