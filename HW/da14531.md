...
ble connection발생시 기본 동작 외에 나머지 동작을 사용자가 설정하는데, 이에 대해 콜백함수를 선언해야 한다.

내부적으로 app_process_catch_rest_cb 이름으로 매크로 함수를 정의해야 한다.
...
app_process_catch_rest_cb 인터페이스는 내부적으로 다음과 같이 정의되어있다.
...
이벤트 핸들러는 4개의 인수를 받는 것을 알 수 있고, 예제 설명을 참조하여 각 인수의 역할을 파악할 수 있다.

```c
/**
 ****************************************************************************************
 * @brief write indication handler.
