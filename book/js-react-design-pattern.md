...
- SSR(Server-Side Rendering)은 서버가 HTML을 렌더링(생성)하고 클라이언트에서 이를 받아 하이드레이션 하는 전통적인 방식이다.

  - SSR은 일반적으로 초기 렌더링 속도에 대한 이점을 챙기기 위해 사용된다.

- SSR은 데이터 fetching, 데이터 연결 작업 등 무거운 작업들을 서버에서 처리하므로, 클라이언트에서 추가적인 데이터 처리를 위한 왕복시간을 줄일 수 있다.

- 서버에서 HTML을 미리 생성하기 때문에 검색엔진 크롤러가 탐색하기에 유리하다. 따라서 SEO 측면에서 좋다.

- CSR기반의 React와 비교해서 클라이언트에서 로드해야할 JS 번들의 크기가 줄어든다.

  - 하지만 번들의 크기가 획기적으로 줄어드는 것은 기대하기 힘들다. 서버측에서 데이터 처리와 HTML 생성만을 담당하므로, 이에 관한 로직은 클라이언트 측에 포함되지 않을 것이다.

  - 클라이언트 또한 하이드레이션을 위한 React 코드를 로드해야 하므로, 서버와 같은 React 코드를 로드한다.

- Next.js의 경우 SSR을 기반으로 한다.

  - 서버는 React 코드를 통해 HTML을 생성(렌더링)하고 클라이언트로 전달한다.

  - 클라이언트 서버로부터 받은 HTML을 통해 빠르게 초기화면을 표시(렌더링)할 수 있으며, React 기반의 JS를 추가로 로드하여 하이드레이션(hydration)을 진행한다.

    - Hydration은 초기 페이지에 동적인 기능(Event handler, state)을 정상적으로 작동하도록 해주는 과정을 말한다.

### 정적 렌더링(SSG)

- 정적 렌더링(Static Site Generation)은 서버의 빌드 시점에 HTML을 미리 생성하여 클라이언트에 HTML을 제공하는 기법이다.

- 빌드 시점에 생성된 HTML파일은 CDN 또는 엣지 네트워크에 의해 캐싱될 수 있으며, 클라이언트 요청시 캐시된 HTML을 제공함으로써 일반적인 SSR 방식보다 응답시간을 단축할 수 있다.

- 정적 렌더링 방식은 누가 요청하든 동일한 데이터를 제공하는 페이지에 적합하다.

- 순수한 정적 렌더링 페이지는 동적 데이터를 포함하지 않는다. 여러 변형된 정적 렌더링 기법을 통해 동적 데이터를 추가하여 페이지를 제공한다.

#### 점진적 정적 생성(ISR)

- ISR(Incremental Static Regeneration)은 정적 렌더링과 SSR을 결합한 방식으로, 특정 정적 페이지만 미리 렌더링하고, 동적 페이지는 일정 주기 또는 사용자 요청 시에 on-demand 방식으로 일부 새로운 HTML을 생성하여 렌더링해서 캐시를 갱신하는 기법이다.

- ISR은 빌드 후 기존 정적 사이트에 점진적으로 업데이트를 적용하여 동적 페이지를 추가한다.

  - 빌드 후 새로운 페이지를 추가하기 위해 지연 로딩(lazy-loading)을 사용한다. 페이지 요청시 서버에서 생성되는 동안 대체 페이지(fallback)를 보여준다.

  - 각 페이지마다 적절한 타임아웃을 정의하고 일정 시간마다 페이지의 유효성을 검사한다. 페이지의 재검증이 완료될 때까지 사용자는 이전 버전의 페이지를 보게 된다. 이러한 방식을 **SWR(Stale-While-Revalidate)** 이라고 한다.

#### On-demand ISR

- On-demand ISR은 기존 ISR방식과 달리 정해진 시간 간격마다 페이지 재검증 과정을 거치지 않고, 특정 이벤트가 발생할 때 페이지를 재생성하는 방식이다.

- 또한 기존 ISR 방식을 통해 재생성된 페이지는 사용자 요청을 처리한 특정 엣지 네트워크 노드에만 캐시된다. On-demand ISR 방식은 모든 엣지 네트워크 노드에 재생성된 페이지를 분배한다. 따라서 모든 사용자가 최신 페이지를 볼 수 있다.

### 스트리밍 SSR

- 스트리밍 SSR은 기존 SSR 방식의 HTML전송을 스트리밍(Streaming)방식으로 전송하는 기법이다. 이를 통해 TTI와 FCP를 단축할 수 있다.

- Node.js의 스트림 기능을 사용해 응답 객체에 데이터를 스트리밍 할 수 있다.

  - HTML데이터를 한 번에 전송하지 않고 데이터를 작은 조각(chunk)로 나눠서 클라이언트로 전송한다.

  - 클라이언트는 데이터 조각을 받는 즉시 콘텐츠 렌더링을 시작할 수 있다.

- 리엑트 18부터 제공되는 **renderToPipeableStream(Node.js 환경) / renderToReadableStream(Edge 환경)** 메서드를 통해서 데이터를 나눠서 전송할 수 있다.

  - 클라이언트는 전달받은 데이터 조각들을 순차적으로 렌더링 할 수 있고, 수신된 DOM 노드에 로드된 React 스크립트에 따라 hydrate 메서드를 호출하여 이벤트 헨들러를 연결하여 상호작용이 가능한 상태로 만들 수 있다.

- 스트리밍은 네트워크 정체 현상에 효과적이다.

  - 네트워크가 혼잡할 때 페이지 전송이 지연될 수 있지만 작은 조각(chunk) 단위로 데이터를 전송하기 때문에 전체 페이지 전송을 완전히 중단하지 않고 부분적으로 빠르게 표시할 수 있다.

  - 결과적으로 열약한 환경에서도 빠른 응답성을 유지할 수 있다.

### 엣지 SSR

- 엣지 SSR(Edge SSR)은 사용자 요청에 대한 응답을 위해 중앙 서버가 SSR을 처리하는 대신, 여러 엣지 서버를 두어 동일한 여러개의 애플리케이션을 배포하고, 사용자로부터 가장 가까운 엣지 서버가 요청을 받아 SSR을 처리하는 기법이다.
...
- 아일랜드 아키텍처(Island Architecture)는 SSR로 만든 정적인 HTML을 서버 사이드에서 렌더링 후 각 구분된 동적 컴포넌트를 **선택적** 으로 하이드레이션 하는 기법이다.

  - 페이지의 대부분은 서버로부터 렌더링된 정적 상태를 유지하고, 필요한 부분만 클라이언트에서 활성화한다.

- 정적인 HTML 파일에는 동적 콘텐츠가 삽입되어야할 여러 동적인 영역이 존재한다. 그 동적 영역을 **아일랜드 컴포넌트** 라고 한다.

  - 각 아일랜드 컴포넌트는 독립된 애플리케이션처럼 동작하며, 서버에서 렌더링된 결과물과 클라이언트에서 컴포넌트를 활성화(Hydration)하는데 사용될 JS코드를 포함한다.

  - 각 아일랜드 컴포넌트는 독립된 JS 번들로 존재하며, 서로 영향을 주지 않는 관계로써 독립되어 있다. 이로 인해 비동기적으로 여러 스크립트를 로드하고 실행해도 충돌이 적고, 전체 페이지 성능이 향상된다.

**점진적 하이드레이션 패턴과 차이점**

- 하이드레이션 범위

  - 점진적 하이드레이션 기법은 전체 페이지를 점진적으로 하이드레이션하는 과정을 거친다.

  - 아일랜드 아키텍처는 각 아일랜드별로 독립적으로 하이드레이션 한다.

- JS 번들 분리

  - 점진적 하이드레이션 기법은 코드 분할이 가능하지만 대부분 단일 앱에서 동적으로 로드 실행되며, 실행순서를 최적화 하여 필요한 부분만 먼저 실행한다.

  - 아일랜드 아키텍처는 **코드 스플리팅(Code Splitting)** 이 적용되어 각 아일랜드 별로 JS 번들을 분리한다.

- 로드 방식

  - 점진적 하이드레이션 방식은 모든 JS 번들을 로드하여 실행 우선순위를 최적화한다.

  - 아일랜드 아키텍처는 **지연 하이드레이션(Lazy Hydration)** 을 지원하여 각 아일랜드가 필요한 시점에 그에 맞는 필요한 JS 번들만 로드하며 실행하므로, 불필요한 JS를 일찍 로드하지 않아도 된다.

**아일랜드 아키텍처 장점**

- 필요한 컴포넌트에 대한 하이드레이션만을 진행하므로, 클라이언트에 전송되는 JS 코드의 양을 줄일 수 있다.

- 모든 정적 콘텐츠가 서버에서 렌더링 되므로 SEO에 유리하다.

- 컴포넌트 기반 아키텍처이므로 아일랜드마다 별도 코드베이스로 관리 가능해 재사용성과 유지보수에 용이하다.
