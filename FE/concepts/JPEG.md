...
**손실 압축** 방식을 사용한다. 따라서 압축 이전에 백업을 하는 것이 좋다.

높은 손실 압축을 사용하기 때문에 이미지의 세부 정보는 상실된다.

### 풀컬러

풀컬러 24bit를 사용하며, 8bit까지 줄일 수 있다.

### 프로그래시브(progressive)

웹 브라우저에서 이미지를 점진적으로 나타나도록 할 수 있다.
...
RGB 색상 공간을 휘도, 채도로 구별함으로써 밝기 정보를 유지할 수 있다.
...
표본을 추출(sampling)하는 과정에서 색차 정보를 줄인다. 따라서 1차적으로 파일 크기가 줄어든다.
...
- J:a:b 형식의 비율로 나눈다.
  ...
  - a: J크기 샘플에서 첫 번째 행의 chroma sampling 수

  - b: J크기 샘플에서 두 번째 행의 chroma sampling 수
...
DCT로 이미지 블록 데이터를 주파수 도메인으로 변환한다.

- 주파수 도메인 값은 압축 효율을 높이는 데 사용된다.

이미지 데이터에 대해 높은 주파수, 낮은 주파수 영역을 찾는다.

- 인접한 픽셀의 편차가 크면 주파수 성분이 높고, 편차가 작으면 주파수 성분이 낮다.

- 경계선, texture 부분은 색상 차이가 크기 때문에 주파수 값이 높게 나오고 배경처럼 고른 부분은 주파수가 낮다.

- 높은 주파수 성분일 수록 값이 0과 가까워진다.

높은 주파수 성분을 찾아 그 부분을 압축하기 위해 사용된다.

- 눈은 높은 주파수 성분에 덜 민감하기 때문에, 해당 부분의 데이터를 줄이거나 없애도 크게 차이가 나지 않는다.

### 4. Quantization - 양자화

DCT를 통해 추출한 각 주파수 성분을 특정 수준으로 반올림해서 크기를 줄인다.

- 이 과정에서 손실압축 의 특성이 발생하며, 데이터가 많이 줄어든다.
...
- 양자화까지 마친 데이터는 연속된 데이터값 패턴이 많기 때문에 RLE로 길이를 효율적으로 최소화 할 수 있다.

- Huffman Coding으로 빈도에 따라 데이터의 길이를 결정하여 크기를 한 번 더 줄인다.
